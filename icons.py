...
_svg_cache: dict[str, str | None] = {}   # icon name -> SVG text (memoized)


def _load_svg(name: str) -> str | None:
    """Load an SVG file's text from the Lucide asset folder.

    Memoized: theme switches re-render ~40 icons and re-reading each file
    from disk every time was measurable (39 x open+read per apply)."""
    hit = _svg_cache.get(name)
    if hit is not None or name in _svg_cache:
        return hit
